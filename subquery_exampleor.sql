...
-- subquery : get info for each actor with film_id = 1 
...
-- first query : find all fiml sissy is in frmo the film_actor table 
select * from film_actor where actor_id = (select actor_id from actor where first_name = 'sissy');


-- second query : find the info on all the films sissy in in 
